...
[Route("api")]
[ApiController]
public class RolesAndPermissionsController(IRolesAndPermissionsManager RolesAndPermissionsManager) : ControllerBase
{
    /// <summary>
    ...
    public IActionResult InsertRole([FromQuery]string roleName)
    {
        ...
        var id = RolesAndPermissionsManager.InsertRole(roleName);
    ...
    public IActionResult InsertPermission(string permissionName)
    {
        ...
        var id = RolesAndPermissionsManager.InsertPermission(permissionName);
        if (id == -1)
            return BadRequest();
        return Ok(id);
    }
